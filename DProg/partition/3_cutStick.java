...
it will be split into two smaller sticks (i.e. the sum of their lengths is 
...
class Solution {
    public int minCost(int n, int[] cuts) {
        // Adding 0 at start and n at end to the cuts array
        ...
        // Sort the cuts array so that left and right subproblems are independent
        Arrays.sort(c);
        ...
        for (int i = c.length - 2; i >= 1; i--) {
            for (int j = i; j <= c.length - 2; j++) {
                int mini = Integer.MAX_VALUE;
                // Try cutting at each position between i and j
                for (int ind = i; ind <= j; ind++) {
                    int cost = c[j + 1] - c[i - 1] + dp[i][ind - 1] + dp[ind + 1][j];
        ...
        // The final answer is the cost of cutting the entire rod from 0 to n
        return dp[1][c.length - 2];
    }
}
